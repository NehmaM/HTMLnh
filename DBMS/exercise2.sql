...
#display data of email and phone_number for all employees
select email,phone_number from employees;

#display data of employee lastname="Fay"
select *from employees where last_name="Fay"; 
-- 1 row(s) returned
-- '202', 'Pat', 'Fay', 'pat.fay@sqltutorial.org', '603.123.6666', '1997-08-17', '11', '6000.00', '201', '2'


#display data of employee whose lastname is "grant" or "whalen"
select hire_date from employees where (last_name="Grant") OR (last_name="Whalen");
-- 2 row(s) returned : '1999-05-24' , '1987-09-17'

#display name of employee who is shipping clerk
select first_name,last_name from employees where job_id=(select job_id from jobs where job_title="shipping clerk");
-- 2 row(s) returned : 'Sarah', 'Bell' , 'Britney', 'Everett'

#display all the employees who work for dept 8
select first_name,last_name from employees where department_id=8;
#6 row(s) returned : 'John', 'Russell' / 'Karen', 'Partners' / 'Jonathon', 'Taylor' /'Jack', 'Livingston'
-- /'Kimberely', 'Grant' / 'Charles', 'Johnson'

#display the dept in descending order
select department_name from departments order by department_name desc;
#11 row(s) returned: 'Shipping'.........'Accounting'

#display all employees whose lastname starts with k
select first_name,last_name from employees where last_name like "k%";
#4 row(s) returned : Steven King , 'Neena' 'Kochhar' , 'Alexander' 'Khoo' , 'Payam' 'Kaufling'

#display name of employees whose hire date are bet 1995 and 1997
SELECT first_name,last_name
FROM employees
WHERE hire_date BETWEEN 1995 AND 1997;  #no result

#display jobs where the max salary is less than 5000
select job_title from jobs where max_salary<5000; #no result

#email address in lowercase
select lower(email) from employees;

#name of employees who hired in 1995
select first_name,last_name from employees where hire_date="1995-05-15"; #no result

#insert paul newton in dept 11
INSERT INTO employees(first_name,last_name,email,phone_number,hire_date,job_id,salary,manager_id,department_id) VALUES ('Paul','Newton','paul.Newton@sqltutorial.org','589.345.2167','1995-05-15',4,22000.00,114,11);
